...
do_wait_for_common(struct completion *x,
		   long (*action)(long), long timeout, int state, int iowait)
{
	if (!x->done) {
		  ...
		  long (*action)(long), long timeout, int state, int iowait)
{
	...
	spin_lock_irq(&x->wait.lock);
	timeout = do_wait_for_common(x, action, timeout, state, iowait);
	spin_unlock_irq(&x->wait.lock);
	return timeout;
}
...
{
	return __wait_for_common(x, schedule_timeout, timeout, state, iowait);
}

static long __sched
wait_for_common_io(struct completion *x, long timeout, int state, int iowait)
{
	return __wait_for_common(x, io_schedule_timeout, timeout, state, iowait);
}
...
{
	wait_for_common_io(x, MAX_SCHEDULE_TIMEOUT, TASK_UNINTERRUPTIBLE, 1);
}
...
{
	return wait_for_common_io(x, timeout, TASK_UNINTERRUPTIBLE, 0);
}
